...
def discriminator_loss(logits_real, logits_fake):
    ...
    Dx = tf.keras.losses.BinaryCrossentropy(from_logits=True)(tf.ones_like(logits_real),logits_real)
    DGz = tf.keras.losses.BinaryCrossentropy(from_logits=True)(tf.zeros_like(logits_fake),logits_fake)
    loss = tf.reduce_mean(Dx) + tf.reduce_mean(DGz)
...
def generator_loss(logits_fake):
    ...
    DGz = tf.keras.losses.BinaryCrossentropy(from_logits=True)(tf.ones_like(logits_fake),logits_fake)
    loss = tf.reduce_mean(DGz)
    ...
    for epoch in range(num_epochs):
        for (x, _) in mnist:
            ...
            if (iter_count % show_every == 0):
                ...
                # imgs_numpy = tf.identity(fake_images) ?
